...
public interface IPedidoRepository extends JpaRepository<Pedido, Long>{
    ...
    @Query("SELECT new com.buensabor.buensabor.dto.cliente.ClientePedidosDto(c.nombre, c.apellido, c.id, COUNT(p)) " +
            "FROM Pedido p JOIN p.cliente c " +
            "WHERE p.fechaPedido BETWEEN :fechaInicio AND :fechaFin " +
            "AND p.sucursal.id = :sucursalId " +
            "AND p.estado = com.buensabor.buensabor.enums.Estado.ENTREGADO " +
